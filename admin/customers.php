<?php
    ...
    // 3. เมื่อมีการส่งค่ามาเพื่อ "ลบข้อมูลลูกค้า"
   if (isset($_GET['delete_id'])) {
    $delete_id = mysqli_real_escape_string($conn, $_GET['delete_id']);
    
    // 1. ดึงรหัสออเดอร์ทั้งหมดของลูกค้านี้มาก่อน
    $res_orders = mysqli_query($conn, "SELECT or_id FROM orders WHERE u_id = '$delete_id'");
    
    while($order = mysqli_fetch_array($res_orders)) {
        $or_id = $order['or_id'];
        // 2. ลบรายละเอียดสินค้าในออเดอร์นั้นๆ
        mysqli_query($conn, "DELETE FROM order_details WHERE or_id = '$or_id'");
    }
    
    // 3. ลบออเดอร์ทั้งหมดของลูกค้านี้
    mysqli_query($conn, "DELETE FROM orders WHERE u_id = '$delete_id'");
    
    // 4. สุดท้ายค่อยลบตัวลูกค้า
    $sql_delete = "DELETE FROM user_login WHERE u_id = '$delete_id'";
    
    if (mysqli_query($conn, $sql_delete)) {
        echo "<script>alert('ลบข้อมูลลูกค้าและประวัติการสั่งซื้อเรียบร้อย'); window.location.href='customers.php';</script>";
        exit;
    }
}
...
<head>
    <meta charset="utf-8">
    <title>จัดการข้อมูลลูกค้า - MEAT SHOP</title>
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                <h2 class="fw-bold text-dark"><i class="fa-solid fa-user-gear text-primary me-2"></i>จัดการข้อมูลลูกค้า</h2>
                <a href="customers.php" class="btn btn-outline-secondary rounded-pill px-4"><i class="fa-solid fa-arrow-left me-2"></i>ย้อนกลับ</a>
            </div>

            <div class="row justify-content-center">
                <div class="col-lg-8">
                    <div class="card mb-4 border-0 shadow-sm p-4 rounded-4">
                        <label class="form-label fw-bold"><i class="fa-solid fa-search me-2"></i>เลือกรายชื่อลูกค้าที่ต้องการแก้ไขหรือลบ</label>
                    ...
                    <div class="card form-card p-4 p-md-5 bg-white <?php echo (!$selected_data) ? 'opacity-50' : ''; ?>">
                        <form action="" method="POST">
                            ...
                            <div class="d-flex flex-wrap gap-3 mt-2">
                                ...
                                <button type="button" class="btn btn-danger py-3 px-4 rounded-3 shadow fw-bold" 
                                        onclick="confirmDelete('<?php echo $selected_data['u_id']; ?>', '<?php echo addslashes($selected_data['u_name']); ?>')">
                                    <i class="fa-solid fa-trash-can me-2"></i>ลบบัญชีลูกค้านี้
                                </button>
                                
                                <a href="customers.php" class="btn btn-light py-3 border px-4 rounded-3">ยกเลิก</a>
                            </div>
                            <?php else: ?>
                            <div class="alert alert-info text-center">กรุณาเลือกรายชื่อลูกค้าจากด้านบนเพื่อเริ่มการจัดการ</div>
                            <?php endif; ?>
                        </form>
                    </div>
                </div>
